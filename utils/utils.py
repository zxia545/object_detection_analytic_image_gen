...
def check_jsonl(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if "answer_format" not in this_json:
                print(f"[ERROR] Skipping invalid JSON line in {file_path}: {line}")
                return False
            else:
                return True
...
def chat_completion(api_base: str, model_name: str, messages: list, max_tokens=256, temperature=0.7, top_p=1):
    """
...
def chat_completion_qwen3(api_base: str, model_name: str, messages: list, max_tokens=256, temperature=0.7, top_p=1):
    if '/v1' not in api_base:
        api_base = api_base + '/v1'
...
def start_vllm_server_with_lora(model_path: str, model_name: str, port: int, lora_path: str, gpu: int = 1):
    command = [
        'python', '-m', 'vllm.entrypoints.openai.api_server',
        f'--model={model_path}',
        f'--served-model-name=lora_base_model',
        f'--tensor-parallel-size={gpu}',
        f'--gpu-memory-utilization=0.85',
        f'--port={port}',
        f'--trust-remote-code',
